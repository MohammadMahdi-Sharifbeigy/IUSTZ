...
double Human::chooseDefItems() {
  ...
  if (check == 0) {
    ...
    cout << items[choice - 1]->getName() << " increased your defense by "
         << synergy << " point" << endl;
    return synergy;
  }
}
...
double Human::chooseAtkItem() {
  ...
  if (check == 0) {
    ...
    if (items[choice - 1]->getID() < 22) {
      synergy = dynamic_cast<Passive*>(items[choice - 1])->getSynergyDamage();
      cout << items[choice - 1]->getName() << " increased your attack by "
           << synergy << " point" << endl;
    } else {
  ...
  if (this->inventorySize() > 0) {
    for (int i = 0; i < inventory.size(); i++) {
      if (inventory[i]->getID() > 28) {
        cout << num << ". " << inventory[i]->getName()
             << " - count:" << inventory[i]->getCount() << " - potency:"
             << dynamic_cast<HealingPotion*>(inventory[i])->getPotency()
             << endl;
        num++;
        check++;
        items.push_back(inventory[i]);
