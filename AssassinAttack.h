...
class AssassinAttack : public AttackStrategy {
 public:
  ...
  return (1.5 * currCharacter->getLevel() * (currCharacter->getMaxHP() / currCharacter->getCurrentHP())
  ...
  return (0.5 * currCharacter->getLevel() * (currCharacter->getMaxHP() / currCharacter->getCurrentHP())
            * ( currCharacter->getDefense()  / currEnemy->get_enemy_atk()));
}

double attackOpponent(Human* currCharacter, Human* opponent){
  // Assassins have very high attack but low defense
  return (1.75 * currCharacter->getLevel() * (currCharacter->getMaxHP() / currCharacter->getCurrentHP())
